...
router.post('/signup', async (ctx) => {
  ...
  if (user.length) { ctx.body = { code: -1, msg: '已被注册' }; return false}
...
router.post('/verify', async (ctx, next) => {
    ...
    await transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
            ...
            Store.hmset(`nodemail: ${ko.user}`, 'code', ko.code, 'expire', ko.expire, ko.email)
        }
    })
    ctx.body = {
        code: 0,
